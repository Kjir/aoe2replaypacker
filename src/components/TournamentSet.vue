...
watch(() => [props.type, props.length], ([newType, newLength], [oldType, oldLength]) => {
  ...
  <div class="mt-4 text-center">
    <ul class="w-full gap-6 flex flex-row justify-center">
      <li class="basis-2/11" v-for="setType in props.setTypes" :key="setType.label()">
        <input name="set-definition" type="radio" v-model="chosenSetDefinition"
          :id="`${setType.type}-${setType.length}`" v-bind:value="setType" class="hidden peer" required />
        <label :for="`${setType.type}-${setType.length}`"
