...
<template>
  <div class="flex flex-col items-center gap-4 p-4 border-2 rounded-lg col-span-3 mt-4">
    ...
    >
      <GameToolbox class="absolute right-4" :game="index" />
      <h3 class="text-center text-2xl">Game {{ index + 1 }}</h3>
      <h4 class="text-center text-lg">{{ game.mapName }}</h4>
      <p v-if="game.date" class="text-center text-sm text-gray-500 dark:text-gray-400">
        <abbr
          :title="
      ...
      <div class="w-full flex flex-row justify-center gap-12">
        <div class="flex items-center gap-4">
          ...
          <CivIcon v-if="game.civ1" :civ="game.civ1.toLowerCase()" class="w-10 h-10 rounded-full" />
        </div>
        <div class="flex items-center gap-4">
          <CivIcon v-if="game.civ2" :civ="game.civ2.toLowerCase()" class="w-10 h-10 rounded-full" />
          <div
            class="w-[150px] text-center overflow-hidden text-ellipsis font-medium dark:text-white"
          >
            <a
